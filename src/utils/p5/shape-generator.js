...
export const generateSeedFromText = (text) => {
  if (!text || typeof text !== 'string') return 12345
  ...
  // Validate final seed and ensure it's finite
  const finalSeed = seed % 10000
  if (!isFinite(finalSeed)) {
    console.warn('Invalid seed generated from text, using fallback')
    return 12345
  }
  ...
  switch (pattern) {
    case 0: // Random distribution
      ...
      for (let i = 0; i < randomCount; i++) {
        const x =
          insetMargin +
          ((seed * (i + 1) * 137) % (canvasWidth - 2 * insetMargin))
        const y =
          insetMargin +
          ((seed * (i + 1) * 73) % (canvasHeight - 2 * insetMargin))
        positions.push({ x, y })
      }
      break
    ...
    case 1: // Grid pattern
      ...
      for (let row = 0; row < gridSize; row++) {
        for (let col = 0; col < gridSize; col++) {
          if ((seed * (row * gridSize + col)) % 3 === 0) {
            // 33% chance per cell
            const x =
              insetMargin +
              col * cellWidth +
              ((seed * (row + col)) % (cellWidth * 0.8))
            const y =
              insetMargin +
              row * cellHeight +
              ((seed * (row * col)) % (cellHeight * 0.8))
            positions.push({ x, y })
          }
        }
      }
    ...
    case 2: // Spiral pattern
      ...
      for (let i = 0; i < spiralCount; i++) {
        ...
        if (
          x >= insetMargin &&
          x <= canvasWidth - insetMargin &&
          y >= insetMargin &&
          y <= canvasHeight - insetMargin
        ) {
          positions.push({ x, y })
        }
      }
    ...
    case 3: // Wave pattern
      ...
      for (let i = 0; i < waveCount; i++) {
        const x =
          insetMargin + (i / (waveCount - 1)) * (canvasWidth - 2 * insetMargin)
...
export const generateShapePositions = (
  ...
  if (
    ...
    console.warn('Invalid inputs to generateShapePositions:', {
      centerX,
      centerY,
      canvasDimensions,
      shapeSeed,
  ...
  const insetMargin = Math.max(
    5,
    Math.min(margin + 30, Math.min(width, height) / 4)
    ...
    case 'cross':
      for (let i = 0; i < numParticles; i++) {
        if (i < 2) {
          ...
          if (isHorizontal) {
            const x =
              insetMargin + ((shapeSeed * i) % (width - 2 * insetMargin))
            const y = centerY + (((shapeSeed * (i + 1)) % 40) - 20)
            positions.push({ x, y })
          } else {
            const x = centerX + (((shapeSeed * (i + 1)) % 40) - 20)
            const y =
              insetMargin + ((shapeSeed * i) % (height - 2 * insetMargin))
            positions.push({ x, y })
          }
        }
      }
  ...
  // Filter out any invalid positions and ensure all coordinates are finite numbers
  ...
  // If no valid positions were generated, return a safe fallback
  if (validPositions.length === 0) {
    console.warn('No valid positions generated, using fallback')
    return [
      { x: Math.min(centerX, width - 50), y: Math.min(centerY, height - 50) },
    ]
  }
...
export const generateTatShapePositions = (
  ...
  if (
    ...
    console.warn('Invalid inputs to generateTatShapePositions:', {
      markovSeed,
      canvasWidth,
      canvasHeight,
    })
  ...
  const safeWidth = Math.max(100, canvasWidth)
  const safeHeight = Math.max(100, canvasHeight)
  const safeMargin = Math.max(
    1,
    Math.min(margin, Math.min(safeWidth, safeHeight) / 4)
  )
  ...
  const tatCount = Math.max(1, Math.floor(safeWidth / 300)) // More Tats for wider canvases
  ...
  for (let tatIndex = 0; tatIndex < tatCount; tatIndex++) {
    ...
    const tatX = (safeWidth / (tatCount + 1)) * (tatIndex + 1)
    const tatY = safeHeight / 2 + (((markovSeed * (tatIndex + 1)) % 200) - 100)
    ...
    for (let i = 0; i < typesCount; i++) {
      ...
      const shapePositions = generateShapePositions(
        ...
        tatY,
        shapeType,
        { width: safeWidth, height: safeHeight }, // Pass safe canvas dimensions
  ...
  const additionalPoints = Math.max(
    5,
    Math.floor((safeWidth * safeHeight) / 50000)
  ) // More points for larger canvases

  for (let i = 0; i < additionalPoints; i++) {
    const seedOffset = markovSeed * (i + 100)
    const x = safeMargin + (seedOffset % (safeWidth - 2 * safeMargin))
    const y = safeMargin + ((seedOffset * 7) % (safeHeight - 2 * safeMargin))
    positions.push({ x, y })
  }

  // Final validation of all positions before returning
  const finalValidPositions = positions.filter((pos) => {
    const isValid =
      pos &&
      typeof pos === 'object' &&
      isFinite(pos.x) &&
    ...
    if (!isValid) {
      console.warn('Invalid position filtered out:', pos)
    }
    return isValid
  })
...
export const generateEnhancedSpawnPositions = (
  ...
  let positions = generateSpawnPositions(
    seed,
    canvasWidth,
    canvasHeight,
    pattern,
