...
test.describe('Blog Posts', () => {
  ...
  test('should navigate to individual blog posts', async ({ page }) => {
    ...
    const postLinks = page.locator(
      'article a, .post a, .blog-post a, [class*="post"] a'
    )
    ...
    if ((await postLinks.count()) > 0) {
      ...
      await expect(
        page.locator('article, .post, .blog-post, [class*="post"]').first()
      ).toBeVisible()
    }
  })
  ...
  test('should display blog post content correctly', async ({ page }) => {
    ...
    if ((await postLinks.count()) > 0) {
      ...
      await expect(
        page.locator('h1, .post-title, .title').first()
      ).toBeVisible()

      // Check that post content is visible
      await expect(
        page.locator('.post-content, .content, .markdown, p').first()
      ).toBeVisible()
